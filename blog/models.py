#from django.db import models
from django.conf import settings
from django.db import models
from django.utils import timezone

# Create your models here.

class Post(models.Model): #models.Model indicate that its a Django model -> saved in database
	author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete = models.CASCADE) #Link to another model
	title = models.CharField(max_length= 200) #Text with limited # of characters
	text = models.TextField() #Long text without limit
	created_date = models.DateTimeField(default = timezone.now)  #Date and time
	published_date = models.DateTimeField(blank = True, null = True)

	def publish(self):
		self.published_date = timezone.now()
		self.save()

	def __str__(self):
		return self.title
